...
name: ["nowa"],
type: ["searching"],
description: "search no whatsapp",
utilisation: userbot.prefix+"nowa",

async execute(m){
let { conn, args, usedPrefix, command } = data
    let teks = args[0]
	if (!teks) throw `Nomornya?\nContoh: ${usedPrefix + command} 628828642xxxx`
	if (!teks.includes('x')) throw `Contoh: ${usedPrefix + command} 628828642xxxx`
